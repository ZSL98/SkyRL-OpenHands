...
def build_runtime_image_in_folder(
    ...
    # NOTE (sumanthrh): we ignore the hash for source files here only for convenience.
    # This assumes that the client and server can interact without having to build a new image for a small change in the source code
    # This allows us to re-use a cached image in the server as long as no critical changes relatd to client <> server interaction are made.
    # For example, you can freely change evaluation code without having to run image caching all over again.
    source_tag = f'{lock_tag}'
